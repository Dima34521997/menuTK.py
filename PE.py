...
# PE - Генератор Перечня Элементов
...
def export_to_word():
    ...
    # Получаем итераторы таблиц в документе и строк в нём
    ...
    try:
        for char in sorted(dict_chars.keys()):
            for component_index, component in enumerate(dict_chars[char]):
                ...
                while True:
                    # Вставляем десигнатор, наименование, кол-во и примечание по одной строке
                    try:
    ...
    final_path = save_dir + new_name + ' ПЭ.docx'
    doc.save(final_path)
    print(f"Файл сохранен по пути: {final_path}")
...
def execute(input_files: list, is_civ: bool):
    ...
    for file in files:
        ...
        common_generator.no_perechen = 0

        print("Получаю данные из перечней элементов...")
        dfs, files_processed = common_generator.get_dfs(dict_chars, [file])

        print("Формирую общую таблицу элементов...")
        dict_chars, prim_not_install, prim_cats, components_one_manuf_categories = \
            common_generator.get_components(dict_chars, dfs, [file])
        ...
        output_path = export_to_word()  # Получаем путь
